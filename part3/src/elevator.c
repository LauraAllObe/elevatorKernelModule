...
int loading(void) {
	...
	if (mutex_lock_interruptible(&thread.mutex2) == 0)
	{
		...
		{
			struct passenger *headcopy = 
				list_first_entry(&elev.floor[elev.current_floor].list, struct passenger, list);
		...
		if((elev.running)||(!list_empty(&elev.list)))
		{
			switch(elev.status)
			{
				case LOADING:
				{
					...
					{
						struct passenger *headcopy = 
							list_first_entry(&elev.list, struct passenger, list);
						elev.current_floor = 
							travel(elev.current_floor, headcopy->destination_floor);	
					} 
					else
					{
						elev.status = IDLE;
...
static ssize_t elevator_read(struct file *file, char __user *ubuf, size_t count, loff_t *ppos) {
	...
	{
		len += snprintf(ptr + len, 4096 - len, "%c%d ", 
			passenger_type_to_char(pass->year), pass->destination_floor+1);
	}
	len += snprintf(ptr + len, 4096 - len, "\n");
	...
	for (int i = 5; i >= 0; i--) 
	{
		...
		{
			len += snprintf(ptr + len, 4096 - len, "%c%d ", 
				passenger_type_to_char(floor_pass->year), floor_pass->destination_floor+1);
		}
		len += snprintf(ptr + len, 4096 - len, "\n");
	}
